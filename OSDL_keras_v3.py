...
if args.mode == 1:
    ...
    combined_filepath="v3/OSDL_keras_v3_combined-{epoch:03d}" + "-{:.2f}.hdf5".format(args.dropout)
    ...
    with open("v3/OSDL_keras_v3_combined_{:.2f}_history.p".format(args.dropout), "wb") as history_pickle:
        pickle.dump(combined_history.history, history_pickle)
    combined_model.save("v3/OSDL_keras_v3_combined_{:.2f}.hdf5".format(args.dropout))
elif args.mode == 2:
    ElMu_model = base_model(len(individual_variables_list))
    ElMu_filepath="v3/OSDL_keras_v3_ElMu-{epoch:03d}" + "-{:.2f}.hdf5".format(args.dropout)
    ...
    with open("v3/OSDL_keras_v3_ElMu_{:.2f}_history.p".format(args.dropout), "wb") as history_pickle:
        pickle.dump(ElMu_history.history, history_pickle)
    ElMu_model.save("v3/OSDL_keras_v3_ElMu_{:.2f}.hdf5".format(args.dropout))
elif args.mode == 3:
    MuMu_model = base_model(len(individual_variables_list))
    MuMu_filepath="v3/OSDL_keras_v3_MuMu-{epoch:03d}" + "-{:.2f}.hdf5".format(args.dropout)
    ...
    with open("v3/OSDL_keras_v3_MuMu_{:.2f}_history.p".format(args.dropout), "wb") as history_pickle:
        pickle.dump(MuMu_history.history, history_pickle)
    MuMu_model.save("v3/OSDL_keras_v3_MuMu_{:.2f}.hdf5".format(args.dropout))