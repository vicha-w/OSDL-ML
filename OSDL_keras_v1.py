...
input_data = input_data[input_data["target"] != 5]
input_data = input_data.sample(frac = 1., random_state=42)

training_size = 1000
training_input = pd.concat([input_data[input_data["target"]==target].iloc[:training_size] for target in range(5)])
training_target = keras.utils.to_categorical(training_input["target"], num_classes=5)
testing_input = pd.concat([input_data[input_data["target"]==target].iloc[training_size:] for target in range(5)])
testing_target = keras.utils.to_categorical(testing_input["target"], num_classes=5)
...
model_layer = Dense(100, activation="relu")(model_layer)
model_layer = Dense(100, activation="relu")(model_layer)
model_layer = Dense(5, activation="softmax")(model_layer)

model = Model(input = [inputs], output = [model_layer])
model.compile(loss="categorical_crossentropy", optimizer="sgd")

filepath="OSDL_keras_v1-{epoch:02d}.hdf5"
...
with open("OSDL_keras_v1_history.p", "wb") as history_pickle:
    pickle.dump(history.history, history_pickle)
model.save("OSDL_model_v1.hdf5")