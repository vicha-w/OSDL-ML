...
os.system("mkdir v4 v4_sorted v4_tanh v4_sorted_tanh")
...
elif args.mode == 3 or args.mode == 4:
...
combined_variables_list = combined_variables_list + ["HT__nom", "HTRat__nom", "HTH__nom", "HTb__nom", "nMediumDeepJetB__nom", "nFTAJet__nom", "sphericity", "isElMu"]
...
if args.mode == 1: prefix_name = "v4/OSDL_keras_v4_"
if args.mode == 2: prefix_name = "v4_tanh/OSDL_keras_v4_tanh_"
if args.mode == 3: prefix_name = "v4_sorted/OSDL_keras_v4_sorted_"
if args.mode == 4: prefix_name = "v4_sorted_tanh/OSDL_keras_v4_sorted_tanh_"

if args.variant == 1:
    combined_model = base_model(len(combined_variables_list))
    combined_filepath = prefix_name + "combined-{epoch:03d}" + "-{:.2f}.hdf5".format(args.dropout)
    ...
    with open(prefix_name + "combined_{:.2f}_history.p".format(args.dropout), "wb") as history_pickle:
        pickle.dump(combined_history.history, history_pickle)
    combined_model.save(prefix_name + "combined_{:.2f}.hdf5".format(args.dropout))
elif args.variant == 2:
    ElMu_model = base_model(len(individual_variables_list))
    ElMu_filepath = prefix_name + "_ElMu-{epoch:03d}" + "-{:.2f}.hdf5".format(args.dropout)
    ...
    with open(prefix_name + "ElMu_{:.2f}_history.p".format(args.dropout), "wb") as history_pickle:
        pickle.dump(ElMu_history.history, history_pickle)
    ElMu_model.save(prefix_name + "ElMu_{:.2f}.hdf5".format(args.dropout))
elif args.variant == 3:
    MuMu_model = base_model(len(individual_variables_list))
    MuMu_filepath = prefix_name + "MuMu-{epoch:03d}" + "-{:.2f}.hdf5".format(args.dropout)
    ...
    with open(prefix_name + "MuMu_{:.2f}_history.p".format(args.dropout), "wb") as history_pickle:
        pickle.dump(MuMu_history.history, history_pickle)
    MuMu_model.save(prefix_name + "MuMu_{:.2f}.hdf5".format(args.dropout))